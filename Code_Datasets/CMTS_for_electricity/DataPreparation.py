...
from CMTS_for_electricity import Utils
...
class DataAccess:
    # attributes:
    # dataFrame: pandas.DataFrame, shape=(rowNum,columnNum)
    # rowNum: int
    # columnNum: int
    ...
    # input:dataFrame.shape=(L,columnNum), input=T, target=I
    # S=ceil((L-T-I+1)/seq_sample_step),  sample(input,target)
    # output: input, shape=(S,columnNum,T)
    #       target, shape=(S,columnNum,I)
    ...
    # output：{category}_input_seq.shape=({category}_size,columnNum,T)
    #       {category}_target_seq.shape=({category}_size,columnNum,I)
    ...
    # {category}_filtered_seq,shape=({category}_size,columnNum,4(STL),T)
    # category in {'train','dev','test'}
...
if __name__ == '__main__':
    df = csv2DataFrame(Const.dataset_name+'.csv', 321)
