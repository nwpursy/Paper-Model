...
from CMTS_for_water.EvaluationMetrics import Evaluation_Utils
...
def train(model, dataAccess, criterion, optimizer, scheduler, option, device):
    ...
    for each_epoch in range(option.epoch):
        ...
        if Const.teacher_forcing_ratio>0.01:
            Const.teacher_forcing_ratio-=0.002
    # plotLossCurve(train_losses, valid_losses)
...
if __name__=='__main__':
    target_time_window=6
    column=12
    log_train = '../resource/Result_Models/CMTS4water/complete/{}h/CMTS@{}.{}.log'.format(
        target_time_window,column,'train')
    log_valid = '../resource/Result_Models/CMTS4water/complete/{}h/CMTS@{}.{}.log'.format(
